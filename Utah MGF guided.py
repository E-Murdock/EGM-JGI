...
Na = 22.989218
H = 1.007276
os.chdir("c:/users/Puri Lab/desktop/metatlas-main")
from metatlas.io import feature_tools as ft

#%%h5 files
os.chdir("c:/users/Puri Lab/downloads/EGM JGI")
...
for i,row in df_files.iterrows():
    ...
    data_setup['file_index'] = row['run_order']
...
os.chdir("c:/users/Puri Lab/desktop/metatlas-main")
from metatlas.io import feature_tools as ft
results = pd.read_csv("c:/users/Puri Lab/downloads/EGM JGI/results_consistentql.csv")
...
for i in range(len(gpos)):
    for j in range(len(gpos.columns)):
        if gpos.iat[i,j] < (background * control.iat[i]):
            gpos.iat[i,j] = 1.022548
...
for i in range(t):
    for j in range(len(gpos.columns)):
        if gpos.iat[i+t,j] < 1000:
            nagpos.iat[i,j] = 1.022548
...
for i in range(len(output)):
    temp = df_files[df_files['run_order'] == output.iat[i,0]]
    output.iat[i,0] = temp.iat[0,0]
...
for i in range(len(naoutput)):
    temp = df_files[df_files['run_order'] == naoutput.iat[i,0]]
    naoutput.iat[i,0] = temp.iat[0,0]